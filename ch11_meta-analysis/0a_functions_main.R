...
assign_i <- function(dat, sims){
  ...
  iters <- parSapply(corei, 1:sims, function(i, ...){assign(dat)})
...
format_balance <- function(balance_output){
  ...
  relabel <- case_when(relabel == "P" ~ "Prior",
                       ...
                       relabel == "m21" ~ "Supported incumbent past election", 
  ...
  tex <- capture.output(print(xtable(out), include.rownames = FALSE))
  tex[5] <- "Baseline covariate & Control mean  & Treatment mean & d-stat & $\\hat{\\beta_1}$ & $\\hat{\\beta_2}$ & N \\\\ "
  n <- length(tex)
  tex[n-2] <- paste0("\\hline Pr($\\chi^2$) &", round(balance_output$p, 2), "&&&&& \\\\ \\hline\\hline") 
  tex[n+1] <- tex[n]
  ifelse(is.na(balance_output$collinear),
         tex[n] <- "\\begin{flushleft}\\textit{Note:} Results show the control and treatment means for each of the pre-treatment covariates. Means and standard deviations are weighted by block share of non-missing observations. $d$-stat is calculated as the difference between treatment and control means normalized by one standard deviation of the control mean. $\\hat{\\beta_1}$ and $\\hat{\\beta_2}$ are the prognostic of the covariate among the control sample on incumbent vote choice and voter turnout, respectively. We also show the probability of rejecting the null that none of the covariates is predictive of treatment. All regressions include block fixed effects, standard errors clustered at the level of assingment and inverse propensity weights, and all countries are weighted equally. $^*$ $p<0.05$; $^{**}$ $p<0.01$; $^{***}$ $p<0.001$. \\end{flushleft}",
         tex[n] <- paste0("\\begin{flushleft}\\textit{Note:} Results show the control and treatment means for each of the pre-treatment covariates. Means and standard deviations are weighted by block share of non-missing observations. $d$-stat is calculated as the difference between treatment and control means normalized by one standard deviation of the control mean. $\\hat{\\beta_1}$ and $\\hat{\\beta_2}$ are the prognostic of the covariate among the control sample on incumbent vote choice and voter turnout, respectively. We also show the probability of rejecting the null that none of the covariates is predictive of treatment. All regressions include block fixed effects, standard errors clustered at the level of assingment and inverse propensity weights, and all countries are weighted equally. Variable(s) ", paste(balance_output$collinear, collapse = ","), " excluded from the F-stat regression because of perfect collinearity with one of more covariates. $^*$ $p<0.05$; $^{**}$ $p<0.01$; $^{***}$ $p<0.001$. \\end{flushleft}"))
  return(tex)
}
