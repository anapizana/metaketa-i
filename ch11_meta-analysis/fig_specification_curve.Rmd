...
prep_graph <- function(mat, args){
  ...
  df$categ <- with(df, case_when(
                        ...
                        categ == "ug1_m10_recode_FALSE" ~ "Uganda 1: News!=0 when P=0", 
                        categ == "ug1_m10_recode_ TRUE" ~ "Uganda 1: News=0 when P=0",
  ...
  df$categ <- factor(df$categ,
                     levels = c(unique(df$categ)[1:13], unique(df$categ)[25:28], setdiff(unique(df$categ)[14:24], "ALL SPECIFICATIONS"), "ALL SPECIFICATIONS"))
  df
}
...
plot_spec_ate <- function(mat, args, title=""){
  ...
  p <- ggplot(data=df) +
    geom_point(aes(y = categ, x = ate, color=sign_95), shape = 3, size = .5) +
    theme_bw() + 
